...
namespace App\Libraries\Feeds\Website\Shop;

use App\Libraries\Feeds\FeedsBase;

class FordMyDinh extends FeedsBase {

    function __construct() {
        parent::__construct();
        $this->base_url = 'http://fordmydinh.org';
    }

    public function getShop(){
    	$url = 'http://fordmydinh.org/san-pham-p7.html';

    	$list = [ ];
        $load_dom = $this->read_url($url);
        $dom_list = $load_dom->find('div[id=ProductList]', 0);
        foreach( $dom_list->find('div[class=ProductImage]') as $item ) {
            $list[] = $item->find('a',0)->href;
        }
        return $list;
    }
